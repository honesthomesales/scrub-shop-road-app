...
// Function to find TSV files
function findTSVFiles() {
    console.log('🔍 Searching for TSV files...');
        ...
        if (fs.existsSync(basePath)) {
            ...
            try {
                ...
                if (tsvFiles.length > 0) {
                    console.log(`✅ Found TSV files in ${basePath}:`);
                    tsvFiles.forEach(file => console.log(`   - ${file}`));
                    return { basePath, tsvFiles };
                }
            } catch (error) {
...
// Main execution
function main() {
    console.log('🚀 TSV File Finder and SQL Generator\n');
    
    const result = findTSVFiles();
    
    if (!result) {
        console.log('\n❌ No TSV files found in common locations.');
        console.log('\n📁 Please check these locations for your TSV files:');
        possiblePaths.forEach(p => console.log(`   - ${p}`));
        console.log('\n💡 Or manually copy your TSV files to the web directory and run this script again.');
        return;
    }
    
    const { basePath, tsvFiles } = result;
    ...
    for (const fileName of tsvFiles) {
        ...
        if (fileName.toLowerCase().includes('venues')) {
            console.log(`\n🏢 Processing venues file: ${fileName}`);
            const venuesSQL = generateSQLFromTSV(filePath, 'venues', [
            ...
            fullSQL += venuesSQL;
        } else if (fileName.toLowerCase().includes('trailer')) {
            console.log(`\n🚛 Processing trailer history file: ${fileName}`);
            const trailerSQL = generateSQLFromTSV(filePath, 'trailer_history', [
                { 
                    source: 'Date', 
                    target: 'date',
                    transform: (value) => {
            ...
            fullSQL += trailerSQL;
        } else if (fileName.toLowerCase().includes('camper')) {
            console.log(`\n🏕️  Processing camper history file: ${fileName}`);
            const camperSQL = generateSQLFromTSV(filePath, 'camper_history', [
                { 
                    source: 'Date', 
                    target: 'date',
                    transform: (value) => {
