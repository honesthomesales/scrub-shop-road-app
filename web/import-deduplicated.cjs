...
// Function to parse TSV file and generate SQL with deduplication
function generateSQLFromTSV(filePath, tableName, columnMapping, deduplicateKey = null) {
    try {
        ...
        if (deduplicateKey) {
            const keyIndex = headers.findIndex(h => 
                h.toLowerCase().includes(deduplicateKey.toLowerCase())
            );
            ...
            if (keyIndex !== -1) {
                ...
                processedLines = uniqueLines;
                console.log(`🔄 Deduplicated: removed ${duplicates} duplicate entries`);
                console.log(`📊 Final count: ${processedLines.length} unique rows`);
            }
        }
        ...
        const insertStatements = processedLines.map((line, index) => {
            const values = line.split('\t');
...
// Main execution
function main() {
    console.log('🚀 TSV Import Script with Deduplication\n');
    
    const outputFile = 'supabase-import-deduplicated.sql';
    let fullSQL = `-- Auto-generated Supabase Import Script with Deduplication\n`;
    ...
    // Process Venues file with deduplication
    ...
    ], 'Promo'); // Deduplicate by Promo column
    fullSQL += venuesSQL;
    ...
    console.log(`🚀 The script will import all your TSV data with deduplication`);
}
