-- Fixed Supabase Authentication Setup for Scrub Shop Road App (Version 2)
...
-- Create policies for users table (FIXED - no infinite recursion)
...
-- Admins can read all user profiles (FIXED - simplified to avoid recursion)
CREATE POLICY "Admins can view all profiles" ON public.users
    FOR SELECT USING (
        (SELECT role FROM public.users WHERE email = auth.jwt() ->> 'email') = 'admin'
    );

-- Admins can update all user profiles (FIXED - simplified to avoid recursion)
