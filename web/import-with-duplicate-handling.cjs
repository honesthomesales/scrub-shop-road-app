...
// Function to parse TSV file and generate SQL with duplicate handling
function generateSQLFromTSV(filePath, tableName, columnMapping, conflictHandling = null) {
    try {
...
// Main execution
function main() {
    console.log('🚀 TSV Import Script with Duplicate Handling\n');
    
    const outputFile = 'supabase-import-with-duplicates.sql';
    let fullSQL = `-- Auto-generated Supabase Import Script with Duplicate Handling\n`;
    fullSQL += `-- Generated on ${new Date().toISOString()}\n\n`;
    
    // Clear existing data (only for sales tables, keep venues)
    fullSQL += `-- Clear existing sales data (keep venues)\n`;
    fullSQL += `TRUNCATE TABLE trailer_history CASCADE;\n`;
    fullSQL += `TRUNCATE TABLE camper_history CASCADE;\n\n`;
    
    // Process Venues file with conflict handling
    ...
    console.log(`🚀 The script will import all your TSV data with duplicate handling`);
}
